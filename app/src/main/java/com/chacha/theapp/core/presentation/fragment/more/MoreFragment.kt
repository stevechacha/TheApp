...
import com.chacha.theapp.databinding.MoreFragmentBinding
...
class MoreFragment : Fragment(R.layout.more_fragment) {
    ...
    private fun setUpObservers() {
        viewModel.uiState.observe(viewLifecycleOwner) {
            when (it) {
                MoreUIState.Loading ->{}  //renderLoading()
        ...
        viewModel.interactions.observeEvent(viewLifecycleOwner) {
            when (it) {
                is MoreActions.Navigate -> findNavController().navigate(it.destination)
            }
        }
    }
